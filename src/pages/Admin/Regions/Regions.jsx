...
export function RegionPage() {
  ...
  const onSubmit = (regionData) => {
    postFetch("region/", regionData, {
  ...
  return (
    <>
      <h1 className="text-4xl text-white font-bold mb-8">Tumanlar</h1>
        ...
        <table className="w-full text-center">
          <thead className="bg-indigo-400">
            <tr className="text-gray-600">
              ...
              <th className="py-2">Shahar va tumanlar nomi</th>
              <th className="py-2">Undagi tashkilotlar soni</th>
            </tr>
          </thead>
          <tbody className="text-white">
            {regionData?.map((region) => {
              return (
                <tr className="border" key={region.id}>
      ...
      {isOpenModal ? (
        <div className="fixed inset-0 flex items-center justify-center bg-black/50 backdrop-blur-sm z-50">
          <div className="bg-gray-800 w-[50%] p-4 overflow-y-auto scroll-none h-max">
            ...
            >
              <Input
                labelTitle="Tuman nomi"
                placeholder="ex. Qarshi tumani"
